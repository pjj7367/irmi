...
export default function KoreaMap() {
  ...
  const regionScoreMap = new Map(
    mockRegions.map((r) => [r.name, r])
  );

  function handleMouseMove(e: React.MouseEvent, name: string) {
    if (!containerRef.current) return;
    const rect = containerRef.current.getBoundingClientRect();
    const data = regionScoreMap.get(name);
    const score = data?.score ?? 50;
    const issue = data?.topIssue ?? "";
    setTooltip({
      name,
      score,
      issue,
  ...
  const regionStyle = (name: string) => {
    ...
    const score = data?.score ?? 50;
    return {
      fill: getRegionFill(score),
      stroke: getRegionStroke(score),
      className: "map-region",
      onMouseMove: (e: React.MouseEvent) => handleMouseMove(e, name),
  ...
  return (
    <div className="panel p-5 h-full" ref={containerRef}>
      ...
      <div className="relative flex justify-center">
        <svg viewBox="100 120 280 470" className="w-full h-auto max-h-[480px]">
          {PROVINCES.map((p) => (
            <path
              key={p.name}
              d={p.d}
          ...
          {[...PROVINCES, ...CITIES].map((item) => {
            ...
            const score = data?.score ?? 50;
            return (
              <g key={`lbl-${item.name}`} className="pointer-events-none select-none">
                <text
                  x={item.lx}
                  y={item.ly - 6}
                  textAnchor="middle"
        ...
        {tooltip && (
          ...
          >
            <div className="flex items-center gap-3 mb-1">
              <span className="font-bold text-foreground text-sm">{tooltip.name}</span>
              <span
              ...
              <span
                ...
                style={{
                  ...
                  backgroundColor: `${getScoreColor(tooltip.score)}15`,
                }}
              >
                {getLevelLabel(tooltip.score)}
              </span>
            </div>
            {tooltip.issue && (
              <div className="text-xs text-text-muted">{tooltip.issue}</div>
            )}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-center gap-5 mt-3 text-xs text-text-muted">
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded-sm" style={{ background: "rgba(239,68,68,0.55)" }} />
          긴급 75+
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded-sm" style={{ background: "rgba(245,158,11,0.5)" }} />
          주의 60+
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded-sm" style={{ background: "rgba(56,189,248,0.4)" }} />
          관찰 45+
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded-sm" style={{ background: "rgba(34,197,94,0.4)" }} />
          안전
        </span>
      </div>
    </div>
